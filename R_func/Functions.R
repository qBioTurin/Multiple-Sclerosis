...
INFg_production_NK<-function(x=NULL, analysis )
{
  if(is.null(x))
  {
...
INFg_production<-function(x=NULL, patient )
{
...
INFgConsuption <- function(analysis,x=NULL)
{
	if(is.null(x))
	{
		if(analysis == "sensitivity"){
...
Cinf<- function(x,patient = "HEALTHY")
{
  p=x[12]
  return(matrix(p, ncol = 1))
}
